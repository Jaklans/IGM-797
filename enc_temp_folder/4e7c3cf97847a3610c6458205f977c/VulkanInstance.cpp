...
static std::vector<char> readFile(const std::string& filename) {
	...
	void VulkanInstance::drawFrame() {
		...
		if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, sentFrameFences[currentFrame]) != VK_SUCCESS) {
	...
	void VulkanInstance::resetCmdBuffer(int index) {
		...
		//vkCmdDraw(commandBuffers[index], static_cast<uint32_t>(triVerts.size()), 1, 0, 0);
